(function (root) {
  ...
  function monthPanel(plan, names, calendar, month, top) {
    ...
    for (let day = 1; day <= total; day++) {
      ...
      s += `<g><title>${escape(key + ': ' + (!inside ? 'Outside selected schedule' : !working ? (calendar.notes[key] || 'Weekend / official day off') : 'Milestone ' + stage.poc))}</title><rect x="${x}" y="${y}" width="146" height="94" rx="8" fill="${color}" stroke="#e2e2e2"/><text x="${x+12}" y="${y+25}" font-size="19" fill="${inside ? '#191919' : '#999'}">${day}</text>`;
